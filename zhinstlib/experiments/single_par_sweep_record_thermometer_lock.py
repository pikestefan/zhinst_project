...
output_voltage = 40e-3  # Units in Volts, resonant driving voltage
...
sweeper_fixed_amplitudes = np.linspace(0.2,0.3, 2) # volts, used when phase is swept
...
for sweeper_fixed_amplitude in sweeper_fixed_amplitudes:
    ...
    daqmodule_name, sus_signal_paths = zi_device.set_subscribe_daq([resonant_demod, thermometer_demod],
    ...
    for ii, sweep_value in enumerate(sweep_values):
        ...
        print(f"Optimizing...")
        freq_out = zi_device.sweep_freq_and_optimize(**optimizer_kwargs)
        res_freqs.append(freq_out)
