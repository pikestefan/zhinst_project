...
resonant_demod0 = 0 # The demodulator used for the resonant driving
resonant_demod1 = 1 # The demodulator used for the resonant driving
sweep_demod = 2 # The demod corresponding to the row to sweep the driving
output_channel = 0
desired_signals = ['x', 'y', 'r', 'phase']
...
sweeper_fixed_amplitude = 10 # volts, used when phase is swept
...
file_directory = r'C:\Users\QMPL\Documents\local_exp_runs\parametric\ndeg08\phase_sweeps\mode_0_3\10V'
file_name = 'stream'
...
zi_device.set_output_volt(resonant_demod0, voltage=output_voltage, column=output_channel)
zi_device.set_output_volt(resonant_demod1, voltage=output_voltage, column=output_channel)
...
if not zi_device.get_demod_output(resonant_demod0, output_channel):
    zi_device.set_demod_output(resonant_demod0, on_state=True, column=output_channel)

if not zi_device.get_demod_output(resonant_demod1, output_channel):
    zi_device.set_demod_output(resonant_demod1, on_state=True, column=output_channel)

daqmodule_name, sus_signal_paths = zi_device.set_subscribe_daq([resonant_demod0, resonant_demod1],
                                                               desired_signals, save_files=save_to_file,
                                                               **calibration_read_kwargs, **saving_kwargs)
print(sus_signal_paths)
zi_device.sync()
...
zi_device.read_daq_module(daqmodule_name, sus_signal_paths, save_to_dictionary=True, save_on_file=save_to_file)
...
for ii, sweep_value in enumerate(sweep_values):
    if sweep_parameter == 'phase':
        zi_device.set_mod_phase(mod_choice='sideband1', phase=sweep_value)
        if ii == 0:
        ...
        if ii == 0:
            zi_device.set_mod_phase(mod_choice='sideband1', phase=sweeper_fixed_phase)
    else:
