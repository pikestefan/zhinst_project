...
class ziVirtualDevice(object):
    ...
    def set_subscribe_daq(
        ...
        demod_paths = [
            self._baseaddress + f"/demods/{demod}/sample" for demod in demods
        ]
    ...
    def read_daq_module(
        self,
        daq_module_name=None,
        signal_paths=None,
        timeout=None,
        remove_atfinish=False,
        save_to_dictionary=True,
        save_on_file=False,
    ):
        """
        ...
        if timeout is None:
            timeout = (
                1.5 * daq_module.getDouble("count") * daq_module.getDouble("duration")
            )
        ...
        while not daq_module.finished() and not self._daqstopRequested:
            if time.time() - tstart > timeout:
                raise Exception(
                    f"Timeout occured after {timeout} seconds. Are streaming nodes enabled?"
                    "Has a valid signal been specified?"
                )
            data_dict = self._read_data_stream(
                daq_module, data_dict, signal_paths, save_to_dictionary
            )
        data_dict = self._read_data_stream(
            daq_module, data_dict, signal_paths, save_to_dictionary
        )
        ...
        if save_on_file:
            ...
            while daq_module.getInt("save/save") != 0:
                ...
                if time.time() - tstart > timeout:
                    raise Exception(
                        f"Timeout after {timeout} second before data save completed."
                    )
...
class moddedziVirtualDevice(ziVirtualDevice):
    def __init__(self, dev_name="", auto_properties=True, *args, **kwargs):
        super(moddedziVirtualDevice, self).__init__(
            dev_name=dev_name, auto_properties=auto_properties, *args, **kwargs
        )
...
class moddedPyQtziVirtualDevice(moddedziVirtualDevice, QObject):
    ...
    def __init__(self, dev_name="", auto_properties=True, *args, **kwargs):
        super(moddedPyQtziVirtualDevice, self).__init__(
            dev_name=dev_name, auto_properties=auto_properties, *args, **kwargs
        )
