...
class PyQtziVirtualDevice(QObject):
    ...
    def __init__(self, dev_name='', auto_properties=True, *args, **kwargs):
        ...
        self.ziServer = zi.ziDAQServer(*args, **kwargs)
        self.dev_name = dev_name
        self.__baseaddress = f'/{dev_name}'
        self._demod_num = self.get_available_demods()
        self.demod_properties = dict().fromkeys([demod for demod in range(self._demod_num)])
        self.clockbase = self.ziServer.getInt(self.__baseaddress + '/clockbase')
        self.daqmodules = dict()
        self._daqstopRequested = False
    ...
    def get_device_nodes(self):
        nodes = self.ziServer.listNodes(self.__baseaddress)
    ...
    def poll_demod_stream(self, demod=None, duration=1, return_amp=False, timeout_ms=100, *args, **kwargs):
        ...
        self.ziServer.flush()
        data = self.ziServer.poll(duration, flat=True, timeout_ms=timeout_ms, *args, **kwargs)
    ...
    def set_subscribe_daq(self, demods=None, signal_list=None, read_duration=1., burst_duration=1.,
        ...
        flags = zi.ziListEnum.recursive | zi.ziListEnum.absolute | zi.ziListEnum.streamingonly
        streaming_nodes = self.ziServer.listNodes(self.__baseaddress, flags)
    ...
    def get_available_demods(self):
        return len(self.ziServer.listNodes(self.__baseaddress + '/demods'))
    ...
    def get_demod_settings(self, demod=None):
        props = self.ziServer.listNodes(self.__baseaddress + f'/demods/{demod}', settingsonly=True)
        prop_dictionary = dict()
        for prop in props:
...
if __name__ == '__main__':
    ...
    dev = PyQtziVirtualDevice(device_id, props['serveraddress'], props['serverport'], props['apilevel'])
    print(dev.get_available_demods())