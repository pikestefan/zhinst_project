...
def remap_ZIfiles_dir(parent_fold, start_stream=None, stop_stream=None, target_dir=None, experiment_name='',
                      init_counter=0):
    """
...
def import_ringdowns(folder_path, signal_demod='0', reference_demod='1', file_type='custom'):
    if type(folder_path) is str:
        folder_path = Path(folder_path)
    if file_type not in ['custom', 'zi']:
        raise Exception("Please provide an accepted file type. Options are 'custom' or 'zi'.")
    ...
    for h5file in h5file_list:
        if file_type == 'custom':
            ...
            signal_name = [signal for signal in data_dictionary.keys() if signal[0] == signal_demod][0]
            ref_name = [signal for signal in data_dictionary.keys() if signal[0] == reference_demod][0]

            signal_array, ref_array = data_dictionary[signal_name], data_dictionary[ref_name]
...
def import_ringdowns_v2(folder_path, signal_demod='0', reference_demod='1', file_type='custom'):
...
if __name__ == '__main__':
    res = get_h5_signals(r'Z:\membrane\SLAB interferometer\data\2021\04\12\DEGPAR02\phase_sweeps\mode0\parametric_100mV\00.h5')
    print(res)
