...
def create_wafer(
    ...
    circ = Circle((0, 0), rad, facecolor=[0.7, 0.7, 0.7])

    clip_size = 0.95 * rad * 2
    rect = Rectangle(
        (-rad, rad - clip_size), 2 * rad, clip_size, facecolor="none", edgecolor="none"
    )
    ax.add_artist(circ)
    ax.add_artist(rect)
    ...
    for ii in range(1, cells[0] + 1):
        for jj in range(1, cells[1] + 1):
            ...
            if jj == 1:
                letter = str(ii)
                ax.text(
                    -sq_edge / 2 - pad_letter,
                    y_corner + single_cell_side / 2,
                    letter,
                    label_kwargs,
                )
            if ii == 1:
...
def get_Q_factor(
    ...
    **kwargs
    ...
    if p0_nogamma is None:
        p0_nogamma = [1,] * len(
            signature(fitting_func).parameters
        ) - 2  # -2 because x axis and decay are excluded
    fit_guess = [gamma_guess] + p0_nogamma
    if fit_guess[0] < 0:
        print("Negative derivative estimated. Avoided fitting.")
        fit_result, Q_factor = None, None
    else:
        fit_result, cov_mat = curve_fit(
            fitting_func, timetrace, decay, p0=fit_guess, *args, **kwargs
        )
